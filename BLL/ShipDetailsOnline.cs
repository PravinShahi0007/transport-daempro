...
{
    [Serializable]
    public class ShipDetailsOnline
    {
        ...
        public bool Add(string ConnectionStr)
        {
            try
            {
                ...
                {
                    myContext.ShipDetailsOnlineInsert(this.Branch,
                                                 this.VouLoc,
                                                 this.VouNo,
                                                 this.FNo,
                                                 this.Service,
        ...
        public bool Delete(string ConnectionStr)
        {
            try
            {
                using (DataERP3DataContext myContext = new DataERP3DataContext(ConnectionStr))
                {
                    myContext.ShipDetailsOnlineDelete(this.Branch, this.VouLoc, this.VouNo);
                    return true;
                }
            }
            catch (Exception)
        ...
        public List<ShipDetailsOnline> GetAll(string ConnectionStr)
        {
            try
            {
                using (DataERP3DataContext myContext = new DataERP3DataContext(ConnectionStr))
                {
                    var result = myContext.ShipDetailsOnlineGet(this.Branch, this.VouLoc, this.VouNo);
                    return (from Sitm in result
                            select new ShipDetailsOnline
                            {
                                Branch = Sitm.Branch,
                                VouLoc = Sitm.VouLoc,
                                VouNo = Sitm.VouNo,
