    ...
    public class ProvidersDetails
    {
        ...
        public bool Add(string ConnectionStr)
        {
            try
            {
                ...
                {
                    myContext.ProvidersDetailsInsert(this.Branch,
                                                 this.VouLoc,
                                                 this.VouNo,
                                                 this.FNo,
                                                 this.Service,
        ...
        public bool Delete(string ConnectionStr)
        {
            try
            {
                using (DataERP3DataContext myContext = new DataERP3DataContext(ConnectionStr))
                {
                    myContext.ProvidersDetailsDelete(this.Branch, this.VouLoc, this.VouNo);
                    return true;
                }
            }
            catch (Exception)
        ...
        public List<ProvidersDetails> GetAll(string ConnectionStr)
        {
            try
            {
                using (DataERP3DataContext myContext = new DataERP3DataContext(ConnectionStr))
                {
                    var result = myContext.ProvidersDetailsGet(this.Branch, this.VouLoc, this.VouNo);
                    return (from Sitm in result
                            select new ProvidersDetails
                            {
                                Branch = Sitm.Branch,
                                VouLoc = Sitm.VouLoc,
                                VouNo = Sitm.VouNo,
